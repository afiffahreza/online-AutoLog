...
retrain = 0 # 0 when post retraining with redis app
...
def loki_scoring(loki_client, app, baseline_time_start, baseline_time_end, log_period, save_path=None):
    ...
    while time_start < baseline_time_end:
        ...
        if app == 'redis-cart' and retrain == 1:
            lines = []
        else:
...
if __name__ == "__main__":
    ...
    applications = os.environ.get('APPLICATIONS', 'frontend cartservice productcatalogservice currencyservice paymentservice shippingservice emailservice checkoutservice recommendationservice adservice redis-cart').split(' ')
    ...
    prefix_output_dir = os.environ.get('PREFIX_OUTPUT_DIR', './post-retrain/')
    ...
    for app in applications:
        ...
        if mode == 'file' and app != 'redis-cart':
